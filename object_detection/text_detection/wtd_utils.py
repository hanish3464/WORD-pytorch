...
import cv2
import math
from text_detection import config
...
def getdetReigion_core(image, textmap, linkmap, text_threshold, link_threshold, low_text, np_x):
    ...
    # det = []
    # mapper = []
    # global_map = np.zeros(textmap.shape, dtype=np.uint8)
    # for k in range(1, nLabels):
    #     # size filtering
    #     size = stats[k, cv2.CC_STAT_AREA]
    #     if size < 10: continue
    ...
    #     mapper.append(k)
    #     image = cv2.resize(image, (256, 256), interpolation=cv2.INTER_LINEAR)
    #     for np_k, d in enumerate(det):
    #         d = np.array(d).astype(int)
    #         xmin, ymin = d[0]; xmax, ymax = d[2]
    #         crop = image[ymin : ymax, xmin : xmax, :]
    #        cv2.imwrite('./res/crop/' + str(np_x) + '-' + str(np_k) + '.jpg', crop)
